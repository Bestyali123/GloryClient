...
public class RoundedUtil {
    ...
    public static ShaderUtil roundedOutlineShader = new ShaderUtil("glory/Shaderss/roundRectOutline.frag");
    private static final ShaderUtil roundedTexturedShader = new ShaderUtil("glory/Shaderss/roundRectTextured.frag");
    ...
    public static void newdrawRound(float x, float y, float width, float height, float radius, Color color,boolean blur,float st) {
        if (blur){
            BlurBuffer.newblurRoundArea(x-0.3f,y-0.3f,width+0.6f,height+0.6f, (int) radius,st);
        }
        drawRound(x, y, width, height, radius, false, color);
    ...
    public static void newdrawGradientHorizontal(float x, float y, float width, float height, float radius, Color left, Color right) {
        drawGradientRound(x, y, width, height, radius, left, left, right, right);
    }
    public static void newdrawGradientVertical(float x, float y, float width, float height, float radius, Color top, Color bottom,boolean blur ,float st) {
        if (blur){
            BlurBuffer.newblurRoundArea(x-0.3f,y-0.3f,width+0.6f,height+0.6f, (int) radius,st);
        }
        drawGradientRound(x, y, width, height, radius, bottom, top, bottom, top);
